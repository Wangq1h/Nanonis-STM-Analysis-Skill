...
def load_signals(path: str | Path) -> tuple[dict[str, Any], str]:
    ...
    if suffix == ".dat":
        ...
        return getattr(nf.obj, "signals", {}), "analystm.nanonis_io.read_nanonis_file"
    if suffix in {".txt", ".csv", ".tsv", ".ibw"}:
        from .import_io import read_imported_file

        imported = read_imported_file(file_path)
        return getattr(imported.obj, "signals", {}), "analystm.import_io.read_imported_file"
