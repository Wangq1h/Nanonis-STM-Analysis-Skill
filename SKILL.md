...
Run `analystm --help`, or `PYTHONPATH=src python -m analystm --help` from a source checkout with dependencies installed, when local execution is available so the public AnalySTM backend is used by default. Run `scripts/resolve_runtime.py --probe` when optional raw-data dependencies are needed; the default probe must not require PySIDAM, PyQt5, or pyqtgraph and reports AI atom detection as a planned integration. Use legacy PySIDAM probing only for explicit regression/source-mapping work. If no cached runtime is ready, inspect `scripts/resolve_runtime.py --bootstrap-command` and use `scripts/bootstrap_runtime.py` only in an isolated user runtime.
...
- AI atom detection: treat the external detector as a planned integration unless it is explicitly installed and requested; use `analystm atom` for scale choice, lattice QC, and human wipe regions. For a 20 nm, 512 px topography, `resize_ratio=1.5` gives 0.0260417 nm per inference pixel and a 0.3515625 nm atom spacing spans 13.5 inference pixels; nearby scales are the first tuning range.
...
- For AnalySTM availability, runtime dependency checks, persistent cached runtimes, explicit legacy PySIDAM discovery, planned AI integration, and default imports, read `references/runtime-bootstrap.md`.
...
- For every raw Nanonis `.sxm` map, read `scan_dir` and declare a target frame before plotting or combining channels. For report-facing `physical_xy` maps, call `analystm.prepare_sxm_map(..., frame="physical_xy")` and plot `view.data_yx` with `origin=view.plot_origin`: `scan_dir="down"` applies `flipud`, `scan_dir="up"` applies no y flip, and backward data additionally applies `fliplr`. Do not pair the legacy `normalize_sxm_direction_map()` result with `origin="lower"`; it is Nanonis-display order for `origin="upper"`.
- Record SXM orientation provenance as `scan_dir`, acquisition `direction`, `target_frame`, `x_flip`, `y_flip`, `plot_origin`, and `orientation_validation`. Do not average forward/backward maps or make coordinate-dependent claims until every input has been transformed into the same verified frame.
...
- For AI atom recognition, first confirm that the external detector integration is explicitly installed and requested; otherwise report it as planned and use the public `analystm atom` scale/QC/wipe helpers. When detector inference is actually used, record parameters including `resize_ratio`, `min_dist`, `prob_threshold`, `patch_size`, `stride`, and preprocessing. After detection, run `analystm atom lattice-qc`; the sites should form an orderly square lattice with few duplicate-like close pairs or vacancy-like gaps. If QC fails, adjust detector parameters and rerun instead of adding a manual review/calibration relabeling workflow.
